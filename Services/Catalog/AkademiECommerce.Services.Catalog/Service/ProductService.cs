...
namespace AkademiECommerce.Services.Catalog.Service
{
    public class ProductService : IProductService
    {
        ...
        {
            var client = new MongoClient(databaseSettings.ConnecitonString);
        ...
        public async Task<ResponseDto<ProductDto>> GetByIdAsync(string id)
        {
            ...
            if (product == null)
            {
                return ResponseDto<ProductDto>.Fail("Kategori bulunamadı", 404);
            }
            else
            {
                return ResponseDto<ProductDto>.Success(_mapper.Map<ProductDto>(product), 200);
